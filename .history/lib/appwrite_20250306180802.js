import {
  ...
  export const getCurrentUser = async () => {
    try {
      ...
      console.log(user)// Fetch the logged-in user details
      return user;
    } catch (error) {
  ...
  export const getOrders = async (userId, role) => {
    try {
      const response = await database.listDocuments(
        config.databaseId,  // Ensure you're passing the correct database ID
        config.productcollectionsId, // Ensure you're passing the correct collection ID
        [
          role === "consumer"
            ? Query.equal("userId", userId)    // Fetch orders placed by the consumer
            : Query.equal("farmerId", userId)  // Fetch orders assigned to the farmer
        ]
      );
  
      return response.documents; // Returns an array of orders
    } catch (error) {
