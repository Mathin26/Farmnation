...
const productListings = [
  ...
  },
  {
    ...
    price: "₹42/kg",
    originalPrice: "₹45/kg",
    image: images.onion, // Using imported images
  ...
  return (
    <SafeAreaView className="bg-background h-full px-4">
      <FlatList
        ...
        columnWrapperStyle={{ gap: 16 }} // Space between columns
        contentContainerStyle={{ paddingBottom: 20 }} // Avoid overlap with navigation bar
        ListHeaderComponent={() => (
          <View className="my-6 space-y-6">
            <Text className="text-2xl font-bold text-primary">Welcome to FarmNation</Text>
            ...
            <View className="bg-primary-200 p-4 rounded-xl">
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-4">
                {marketPrices.map((item) => (
                  <View key={item.id} className="bg-white p-3 rounded-lg shadow-md">
        ...
        ListEmptyComponent={() => (
          <EmptyState
            ...
            subtitle="Wanna see some Analytics about the Commodities price"
          />
        )}
